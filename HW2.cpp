...
const float PI = 3.1415926536f; /*Pi value for drawing cone*/
...
GLuint	BoxList;				// heli body display list
GLuint  BladeList;				// heli blade display list
GLuint  ConeList;				// front object display list
int		MainWindow;				// window id for main graphics window
float	Scale;					// scaling factor
int		ShadowsOn;				// != 0 means to turn shadows on
int		ViewOn;					// switch inside/outside view 
...
Display( )
{
	...
	if (ViewOn != 0)
	{
		gluLookAt(-0.4, 1.8, -4.9, 0., 0., -15., 0., 1., 0.);/*inside view*/
	}
	else
	{
		gluLookAt(-10., 5., 10., 0., 0., 0., 0., 1., 0.);/*outside view*/
	...
	glRotatef(90, 0., 1., 0.);
	glRotatef(Time * 360 * 3, 0., 0., 1.);	/* 3 times faster than top blade*/
...
InitMenus( )
{
	...
	int viewmenu = glutCreateMenu(DoViewMenu); /*create view menu*/
...
InitLists( )
{
	...
	float R = 4;
	glColor3f(1., 0., 0.);
	for (x = 0; x <= 1 * m / piece; x++)
	{
		glVertex3f(0, 2.f, 0);
		glVertex3f(R * cos(2 * PI / m * x), 0.f, R * sin(2 * PI / m * x));
	}
	glColor3f(1., 1., 0.);
	for (x = 1 * m / piece; x <= 2 * m / piece; x++)
	{
		glVertex3f(0, 2.f, 0);
		glVertex3f(R * cos(2 * PI / m * x), 0.f, R * sin(2 * PI / m * x));
	}
	glColor3f(0., 1., 0.);
	for (x = 2 * m / piece; x <= 3 * m / piece; x++)
	{
		glVertex3f(0, 2.f, 0);
		glVertex3f(R * cos(2 * PI / m * x), 0.f, R * sin(2 * PI / m * x));
	}
	glColor3f(0., 0., 1.);
	for (x = 3 * m / piece; x <= 4 * m / piece; x++)
	{
		glVertex3f(0, 2.f, 0);
		glVertex3f(R * cos(2 * PI / m * x), 0.f, R * sin(2 * PI / m * x));
	}
	...
	for (x = 0; x <= m; x++)
	{
		glVertex3f(R / 2 * cos(2 * PI / m * x), 0.f, R / 2 * sin(2 * PI / m * x));
		glVertex3f(0, 0.f, 0);
	}
	glColor3f(1., 0., 1.);
	for (x = 0; x <= m; x++)
